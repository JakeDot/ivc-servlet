...
export const ConnectionStringManager: React.FC<ConnectionStringManagerProps> = ({
  ...
  const [formRawString, setFormRawString] = useState('https://+15550199283@whatsapp.net');
  ...
  const presets = [
    {
      label: 'WhatsApp (+number@whatsapp.net)',
      value: 'https://+15550199283@whatsapp.net',
      cat: 'whatsapp' as ServiceCategory,
      desc: 'WhatsApp Direct Node Bridge',
    },
    {
      label: 'Social Email (user@email.host/service.social)',
      value: 'operator@enterprise.com/whatsapp.social',
      cat: 'social_email' as ServiceCategory,
      desc: 'Federated Social Messaging Relay',
    },
    {
      label: 'Self Identity ($me)',
      value: '$me',
      cat: 'self' as ServiceCategory,
      desc: 'Active Operator Self Identity Pointer',
    },
    {
      label: 'Self Identity Audio ($me/audio)',
      value: '$me/audio',
      cat: 'self' as ServiceCategory,
      desc: 'Local Operator Audio Device Endpoint',
    },
    {
      label: 'IVC Protocol Channel (ivc://...)',
      value: 'ivc://IVC.cx+Sn/$opers',
      cat: 'ivc_protocol' as ServiceCategory,
      desc: 'IVC Operations Broadcast Channel',
    },
    {
      label: 'gRPC High Speed Node (grpc://...)',
      value: 'grpc://mesh.ivc.internal:9090/v1/stream',
      cat: 'grpc' as ServiceCategory,
      desc: 'Internal Microservice Mesh gRPC Stream',
    },
  ];
  ...
  const handleRestoreDefaults = () => {
    ...
    setConnectionNotice('Restored default connection strings.');
    setTimeout(() => setConnectionNotice(null), 3500);
  };

  // Disconnect & Reconnect to alias $me (localhost)
  const handleDisconnectAndReconnectToMe = () => {
    const meItem: ConnectionStringItem = {
      id: `conn-me-${Date.now()}`,
      rawString: '$me',
      label: 'Self Entity ($me / localhost)',
      category: 'self',
      status: 'connected',
      description: 'Reconnected to local alias entity $me (localhost loopback).',
    ...
    setSearchQuery('');
    setSelectedCategory('all');
    setConnectionNotice('Disconnected current connections and reconnected to alias $me (localhost).');
  ...
  const handleOpenAddModal = () => {
    setEditingItem(null);
    setFormRawString('https://+15550199283@whatsapp.net');
    setFormLabel('New WhatsApp Service Node');
    setFormCategory('whatsapp');
    setFormDescription('WhatsApp direct connection string endpoint');
    setIsModalOpen(true);
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
            ...
            <div className="flex items-center space-x-3 mb-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Manage, parse, and validate complex URI schemes, social addresses (<span className="text-emerald-400 font-mono">https://+number@whatsapp.net</span>, <span className="text-blue-400 font-mono">user@email.host/service.social</span>, <span className="text-purple-400 font-mono">$me</span>), and protocol bridges.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              title="Disconnect current services and reconnect to alias $me (localhost)"
            >
              <Power className="w-3.5 h-3.5 text-purple-400" />
              <span>Reconnect to $me (localhost)</span>
            </button>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800/80 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search connection strings, addresses, or $me..."
              value={searchQuery}
          ...
          <div className="max-w-md mx-auto space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">
                {items.length === 0
                  ? 'All connection strings have been disconnected (<empty>). You can disconnect & reconnect directly to alias $me (localhost) or restore the default service list.'
            ...
            <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
              ...
              >
                ...
                <span>Reconnect to alias $me (localhost)</span>
              </button>
            ...
            <div className="mt-6 pt-6 border-t border-slate-800/80 text-[11px] text-slate-500 font-mono text-left bg-slate-950 p-3 rounded-lg border border-slate-800/50">
              <span className="text-slate-400 font-semibold block mb-1">Expected Default Connection String Formats:</span>
              <ul className="space-y-0.5 list-disc list-inside text-slate-400">
                <li><code className="text-emerald-400">https://+number@whatsapp.net</code></li>
                <li><code className="text-blue-400">user@email.host/service.social</code></li>
                <li><code className="text-purple-400">$me</code> (localhost loopback)</li>
                <li><code className="text-indigo-400">ivc://IVC.cx+Sn/$opers</code></li>
              </ul>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSaveModal} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1 font-medium">
                  Connection String Address / URI
                </label>
                <input
                  type="text"
                  required
                  placeholder="https://+15550199283@whatsapp.net or user@email.host/service.social or $me"
                  value={formRawString}
              ...
              <div className="bg-slate-950 border border-slate-800/80 rounded-lg p-3 space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                  ...
                  <div>
                    <span className="text-slate-500">User/Auth:</span>{' '}
                    <span className="text-emerald-300">{liveParsed.user || '<none>'}</span>
                  </div>
                  <div>
                    <span className="text-slate-500">Host:</span>{' '}
                    <span className="text-blue-300">{liveParsed.host || '<none>'}</span>
                  </div>
                  <div>
                    <span className="text-slate-500">Path:</span>{' '}
                    <span className="text-amber-300">{liveParsed.path || '/'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. WhatsApp Ingress Gateway"
                    value={formLabel}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-slate-200 focus:outline-none focus:border-indigo-500"
                  >
                    <option value="whatsapp">WhatsApp (https://+number@whatsapp.net)</option>
                    <option value="social_email">Social Email (user@email.host/service.social)</option>
                    <option value="self">Self Variable ($me)</option>
                    <option value="ivc_protocol">IVC Protocol (ivc://...)</option>
